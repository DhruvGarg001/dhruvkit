...
def cmd_init(args: list):
    ...
    Usage:
        dhruvkit init [--template|-t TEMPLATE]
    ...
    if "--help" in args or "-h" in args:
        ...
        cmd_docs(["dhruvkit", "docs", "init"])
        return
    
    console.print(Panel("[bold blue]🚀 DhruvKit - Initialize Project[/bold blue]", style="blue"))
    ...
    console.print(f"\n[yellow]Initializing project with template:[/yellow] [cyan]{template['name']}[/cyan]")
    if addons_to_apply:
    ...
    display_created_files(folder_name, created_items, is_new=False)
    
    console.print(Panel(
        f"[bold green]✅ Success![/bold green]\n\n"
        f"Project initialized successfully in current directory.\n\n"
        f"Next steps:\n"
